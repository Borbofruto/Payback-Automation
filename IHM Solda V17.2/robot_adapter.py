...
# - pré-limite do workspace: 50 mm antes do limite, jog linear cai para 5 mm/s;
# - joystick ignorado durante trajetória, sem mandar jog_stop;
# - watchdog de emergência/parada protetiva para forçar solda OFF;
...
WORKSPACE_PRE_LIMIT_ZONE_MM = 50.0
WORKSPACE_PRE_LIMIT_SPEED_MM_S = 5.0
...
def _coerce_pose(value: Any, depth: int = 0, origem: str = "sdk") -> Optional[List[float]]:
    ...
    if isinstance(value, dict):
        keys = ("tcp", "pose", "tcp_position", "tool_pos", "tool_position", "cartesian", "cartesian_pose", "cartesiantran_position", "actual_tcp_pose", "tcp_pos", "pos")
        for key in keys:
            if key in value:
...
def _workspace_default() -> Dict[str, Any]:
    return {"enabled": False, "p1": None, "p2": None, "z_margin_mm": 0.5, "xy_margin_mm": 5.0, "slow_zone_mm": WORKSPACE_PRE_LIMIT_ZONE_MM}
...
def _cap_abs(v: float, limit: float) -> float:
    return v if abs(v) <= limit else math.copysign(limit, v)
...
def _workspace_limits_from_cfg(cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        "mode": "oriented_from_far_edge_and_robot_base", "p1_xy": a, "p2_xy": b, "origin_xy": origin,
        "projection_origin_on_edge": q, "near_p1_xy": near_a, "near_p2_xy": near_b, "vertices_xy": verts,
        "u_edge": u, "n_depth": n, "edge_length_mm": edge_len, "depth_mm": depth, "s_origin_mm": s_origin,
        "s_min": 0.0, "s_max": edge_len, "t_min": 0.0, "t_max": depth,
        "s_min_safe": xy_margin, "s_max_safe": edge_len - xy_margin,
        "t_min_safe": xy_margin, "t_max_safe": depth - xy_margin,
        "z_p1": float(p1[2]), "z_p2": float(p2[2]), "z_surface_source": "min_p1_p2",
        "z_surface": z_surface, "z_min_tcp": z_surface + z_margin,
        "xy_margin_mm": xy_margin, "z_margin_mm": z_margin,
        "slow_zone_mm": WORKSPACE_PRE_LIMIT_ZONE_MM,
        "pre_limit_zone_mm": WORKSPACE_PRE_LIMIT_ZONE_MM,
        "pre_limit_speed_mm_s": WORKSPACE_PRE_LIMIT_SPEED_MM_S,
        "x_min": min(xs), "x_max": max(xs), "y_min": min(ys), "y_max": max(ys),
        "x_min_safe": min(xs) + xy_margin, "x_max_safe": max(xs) - xy_margin,
        "y_min_safe": min(ys) + xy_margin, "y_max_safe": max(ys) - xy_margin,
    }
...
def _workspace_status(self) -> Dict[str, Any]:
    ...
    tcp = self._copy_tcp()
    status = {"enabled": bool(cfg.get("enabled")), "configured": limits is not None, "inside": True, "jog_locked": False, "outside_axes": [], "message": "Workspace desabilitado ou não configurado.", "limits": limits}
    if not status["enabled"]:
        status["message"] = "Workspace desabilitado."
        return status
    if limits is None:
        status.update({"inside": False, "jog_locked": True, "message": "Workspace habilitado, mas P1/P2 não formam uma área geométrica válida."})
        return status
    ...
    status["message"] = "TCP dentro da área de trabalho." if status["inside"] else "TCP fora da área de trabalho. Controle aceita apenas movimento de retorno."
    return status
...
def _set_workspace_config(self, dados):
    ...
    for key, lo, hi, default in (("z_margin_mm", 0.0, 300.0, 0.5), ("xy_margin_mm", 0.0, 300.0, 5.0)):
        if key in dados:
            ...
            cfg[key] = default
    cfg["slow_zone_mm"] = WORKSPACE_PRE_LIMIT_ZONE_MM
...
def _bloquear_jog_workspace(self, motivo: str, eixo: int, vel: float) -> float:
    ...
    with self._state_lock:
        ...
        self.diagnosticos["workspace_bloqueio"] = {"motivo": motivo, "eixo": int(eixo), "vel_original": float(vel), "ts": time.time()}
    print(f"[WORKSPACE] Jog bloqueado: {motivo}")
    return 0.0


def _movimento_retorna_para_workspace(outside_axes: List[str], ds: float, dt: float, dz: float) -> bool:
...
def _limitar_velocidade_workspace(self, eixo: int, vel: float) -> float:
    ...
    s, t = _workspace_coords_xy(limits, x, y)
    if eixo == 0:
        move_vec, dz = [vel, 0.0], 0.0
    elif eixo == 1:
        move_vec, dz = [0.0, vel], 0.0
    else:
        move_vec, dz = [0.0, 0.0], vel
    ds = _dot2(move_vec, limits["u_edge"])
    dt = _dot2(move_vec, limits["n_depth"])
    status = self._workspace_status()
    outside_axes = status.get("outside_axes", [])
    if outside_axes:
        if not _movimento_retorna_para_workspace(outside_axes, ds, dt, dz):
            return self._bloquear_jog_workspace("TCP fora da área; movimento não retorna ao workspace", eixo, vel)
        with self._state_lock:
            self.diagnosticos["workspace_pre_limit_active"] = True
            self.diagnosticos["workspace_pre_limit_reason"] = "retorno_de_fora"
        return _cap_abs(vel, WORKSPACE_PRE_LIMIT_SPEED_MM_S)
    self._workspace_stop_emitido = False
    near_boundary = False
    reason = []
    if eixo == 2:
        if vel < 0:
            dist = z - float(limits["z_min_tcp"])
            if dist <= 0:
                return self._bloquear_jog_workspace(f"limite Z atingido: TCP Z={z:.3f}, limite={limits['z_min_tcp']:.3f}", eixo, vel)
            if dist <= WORKSPACE_PRE_LIMIT_ZONE_MM:
                near_boundary = True
                reason.append(f"Z {dist:.1f}mm")
    else:
        if ds < 0:
            dist = s - limits["s_min_safe"]
            if dist <= 0:
                return self._bloquear_jog_workspace("limite S- atingido", eixo, vel)
            if dist <= WORKSPACE_PRE_LIMIT_ZONE_MM:
                near_boundary = True
                reason.append(f"S- {dist:.1f}mm")
        elif ds > 0:
            dist = limits["s_max_safe"] - s
            if dist <= 0:
                return self._bloquear_jog_workspace("limite S+ atingido", eixo, vel)
            if dist <= WORKSPACE_PRE_LIMIT_ZONE_MM:
                near_boundary = True
                reason.append(f"S+ {dist:.1f}mm")
        if dt < 0:
            dist = t - limits["t_min_safe"]
            if dist <= 0:
                return self._bloquear_jog_workspace("limite T- atingido", eixo, vel)
            if dist <= WORKSPACE_PRE_LIMIT_ZONE_MM:
                near_boundary = True
                reason.append(f"T- {dist:.1f}mm")
        elif dt > 0:
            dist = limits["t_max_safe"] - t
            if dist <= 0:
                return self._bloquear_jog_workspace("limite T+ atingido", eixo, vel)
            if dist <= WORKSPACE_PRE_LIMIT_ZONE_MM:
                near_boundary = True
    ...
    if near_boundary:
        with self._state_lock:
            self.diagnosticos["workspace_pre_limit_active"] = True
            self.diagnosticos["workspace_pre_limit_reason"] = ", ".join(reason)
        return _cap_abs(vel, WORKSPACE_PRE_LIMIT_SPEED_MM_S)
    with self._state_lock:
        self.diagnosticos["workspace_pre_limit_active"] = False
        self.diagnosticos["workspace_pre_limit_reason"] = ""
    return vel
...
def _apply_tcp_tool_calibration(self, force=False) -> bool:
    if not TCP_TOOL_CALIBRATION_ENABLED or getattr(self, "modo_simulacao", True):
        return False
    ...
    attempts = (("set_tool_data", (TCP_TOOL_ID, pose)), ("set_tool_data", (pose,)), ("set_tool", (TCP_TOOL_ID, pose)), ("set_tool", (pose,)), ("set_tcp", (pose,)), ("set_tool_frame", (TCP_TOOL_ID, pose)), ("set_current_tool_data", (pose,)), ("set_tool_id", (TCP_TOOL_ID,)))
    errors = []
...
def _robot_fault_reasons_from_diag(self) -> List[str]:
    ...
    try:
        ...
        if code not in (None, "", 0, "0"):
            reasons.append(f"codigo_erro={code}")
    for key, label in (("status_emergencia", "emergência"), ("protective_stop", "protective_stop"), ("driver_sem_robo", "driver_sem_robo"), ("robot_status_bad_ret", "robot_status_bad_ret"), ("robot_status_probe_failed", "robot_status_probe_failed")):
...
def _start_trajectory_fault_watchdog(self):
    ...
    th.start()
    print("[SEGURANÇA] Watchdog de emergência/parada protetiva iniciado (50 ms).")
...
def _patched_snapshot_state(self):
    ...
    dados["trajectory_safety"] = {
        ...
        "workspace_pre_limit_active": bool(self.diagnosticos.get("workspace_pre_limit_active", False)),
        "workspace_pre_limit_reason": self.diagnosticos.get("workspace_pre_limit_reason", ""),
    }
    return dados
...
def _patched_enviar_jog(self, eixo, vel, coord):
    ...
    with self._state_lock:
        self.diagnosticos["jog_soft_start_factor"] = 1.0
